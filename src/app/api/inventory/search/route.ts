...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (bodyText) {
        ...
        if (callFromMessage) {
          ...
          vapiArgs = callFromMessage.arguments || callFromMessage.function?.parameters || {};
        } else if (callFromToolCall && (callFromToolCall.arguments || callFromToolCall.function?.parameters)) {
          isVapi = true;
          toolCallId = callFromToolCall.id;
          vapiArgs = callFromToolCall.arguments || callFromToolCall.function?.parameters || {};
        } else if (directArgs) {
          isVapi = true;
          ...
          vapiArgs = { q: bodyJson.q, limit: bodyJson.limit, cursor: bodyJson.cursor };
        }
      }
    } catch (_) {}
    ...
    const limit = Math.min(parseInt(String(isVapi ? (vapiArgs.limit ?? 5) : (searchParams.get("limit") || "20"))), 50);
    const cursor = (isVapi ? vapiArgs.cursor : searchParams.get("cursor")) || null;
    ...
    if (!isVapi) {
      ...
      if (availableForSale !== null) filters.push(`available_for_sale:${availableForSale}`);
      if (filters.length > 0) searchQuery = `${query} ${filters.join(" ")}`.trim();
    }
    ...
    const transformedProducts = products.edges.map(({ node }: { node: any }) => {
      ...
      const hasAvailableVariant = node.variants.edges.some(({ node: v }: { node: any }) => Boolean(v.availableForSale) || (v.quantityAvailable ?? 0) > 0);
      return {
        id: node.id,
        title: node.title,
        handle: node.handle,
        ...
        vendor: node.vendor,
        priceRange: { min: minPrice, max: node.priceRange.maxVariantPrice.amount, currency },
        ...
        images: node.images.edges.map(({ node: image }: { node: any }) => ({ url: image.url, altText: image.altText })),
      };
    });
    ...
    if (isVapi) {
      ...
      return NextResponse.json({ results: [{ toolCallId: toolCallId || "unknown", result: serialized }] }, { headers: corsHeaders(request.headers.get("origin") || undefined) });
    }

    return NextResponse.json({
      products: transformedProducts,
      pagination: { hasNextPage: products.pageInfo.hasNextPage, endCursor: products.pageInfo.endCursor, totalCount: transformedProducts.length },
    }, { headers: corsHeaders(request.headers.get("origin") || undefined) });
  } catch (error) {
    console.error("Error searching products:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500, headers: corsHeaders(request.headers.get("origin") || undefined) });
  }
}
