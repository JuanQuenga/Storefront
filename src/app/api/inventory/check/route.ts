...
export async function GET(request: NextRequest) {
  try {
    ...
    const variantIds = ids.split(',').map(id => id.trim()).filter(id => id);
    ...
    const response = await shopifyClient.request(INVENTORY_QUERY, {
      ids: formattedIds,
    });
    ...
    const inventoryData = response.data.nodes.map((node: { id: string; sku: string; title: string; inventoryQuantity: number; availableForSale: boolean; price: { amount: string; currencyCode: string }; product: { title: string; handle: string } }) => ({
      id: node.id,
      sku: node.sku,
      title: node.title,
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        inStock: inventoryData.filter(item => item.inventoryQuantity > 0).length,
        outOfStock: inventoryData.filter(item => item.inventoryQuantity === 0).length,
        lowStock: inventoryData.filter(item => item.inventoryQuantity > 0 && item.inventoryQuantity <= 5).length,
      },
    });
