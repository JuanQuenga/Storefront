...
export async function GET(
  ...
  try {
    ...
    const transformedProduct = {
      ...
      variants: product.variants.edges.map(({ node: variant }: { node: any }) => ({
        id: variant.id,
        title: variant.title,
        sku: variant.sku,
      ...
      collections: product.collections.edges.map(({ node: collection }: { node: any }) => ({
        id: collection.id,
        title: collection.title,
        handle: collection.handle,
      })),
      totalVariants: product.variants.edges.length,
      inStock: product.variants.edges.some(({ node }: { node: any }) => node.inventoryQuantity > 0),
    };
